...
if "hypermode.app" in HYPERMODE_ENDPOINT:
    assert (
        "HYPERMODE_API_TOKEN" in os.environ
    ), "HYPERMODE_API_TOKEN environment variable must be defined. You can find the API Token in the Hypermode console under Settings > API Keys."
...
def addDocument(id, filename, namespace=""):
    with open(filename) as f:
        ...
        if "errors" in resp:
            ...
            print(
                f"Document {id} added successfully in namespace {'default' if namespace == '' else namespace}. {len(resp.get('data').get('addMarkdownDocument'))} chunks added."
            )
...
if len(sys.argv) == 2:
    ...
    for file_path, _subdirs, files in os.walk(path):
